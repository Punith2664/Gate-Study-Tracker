document.addEventListener('DOMContentLoaded', () => {
  ...
  measurementId: "G-LD1NMTVL0C"
    ...
    function startTimer() {
        alertSound.play().then(() => alertSound.pause()).catch(() => {}); // Prime audio
    ...
    function updateWorkoutStats() {
        const history = [...new Set((appData.workouts || []))].map(d => new Date(d)).sort((a, b) => a - b);
        const uniqueDays = history.length;
        if (uniqueDays === 0) {
            ["stats-workout-current", "stats-workout-longest", "stats-workout-total"].forEach(id => { if (document.getElementById(id)) document.getElementById(id).textContent = "0 days" });
            return;
        }
        ...
        for (let i = 1; i < uniqueDays; i++) {
            ...
            if (diff === 1) { tempStreak++ } else if (diff > 1) { tempStreak = 1 }
            if (tempStreak > longestStreak) longestStreak = tempStreak;
        }
        const today = new Date(new Date().toISOString().slice(0, 10));
        const lastWorkout = history[uniqueDays - 1];
        if ((today - lastWorkout) / (1000 * 3600 * 24) < 2) { currentStreak = tempStreak }
        document.getElementById('stats-workout-current').textContent = `${currentStreak} day${currentStreak !== 1 ? 's' : ''}`;
        document.getElementById('stats-workout-longest').textContent = `${longestStreak} day${longestStreak !== 1 ? 's' : ''}`;
        document.getElementById('stats-workout-total').textContent = `${uniqueDays} day${uniqueDays !== 1 ? 's' : ''}`;
    }
