document.addEventListener('DOMContentLoaded', () => {
    ...
    function startTimer() {
        pauseTimer(); const n = Date.now(), t = n + timeLeft * 1000; displayTimeLeft(timeLeft);
        countdown = setInterval(() => {
            const s = Math.round((t - Date.now()) / 1000);
            if (s < 0) { clearInterval(countdown); alert("Time for a break!"); logStudySession(appData.customMinutes * 60); setTimer(); return; }
            timeLeft = s; displayTimeLeft(s);
        }, 1000);
    }
    ...
    function updateWorkoutDisplay() {
        ...
        workoutCheckbox.checked = (appData.workouts || []).includes(todayStr);
    }
    workoutCheckbox.addEventListener('change', () => {
        const todayStr = new Date().toISOString().slice(0, 10);
        if (workoutCheckbox.checked) {
            if (!appData.workouts.includes(todayStr)) appData.workouts.push(todayStr);
        } else {
            appData.workouts = appData.workouts.filter(date => date !== todayStr);
        }
        saveData();
    });
    ...
    function updateLoginStats() {
        const history = (appData.loginHistory || []).map(dString => new Date(new Date(dString).toISOString().slice(0, 10))).sort((a, b) => a - b);
        const uniqueHistory = [...new Set(history.map(d => d.getTime()))].map(t => new Date(t));
        if (uniqueHistory.length === 0) return;
        let currentStreak = 0;
        const today = new Date(new Date().toISOString().slice(0, 10));
        let tempDate = new Date(today);
        const uniqueTimeStamps = uniqueHistory.map(d => d.getTime());
        if (uniqueTimeStamps.includes(tempDate.getTime())) {
            currentStreak = 1;
            while (true) {
        ...
        if (uniqueHistory.length > 0) {
            ...
            for (let i = 1; i < uniqueHistory.length; i++) {
                const dayBefore = new Date(uniqueHistory[i]);
                dayBefore.setDate(dayBefore.getDate() - 1);
                if (uniqueHistory[i-1] && dayBefore.getTime() === uniqueHistory[i-1].getTime()) {
                    currentLongest++;
                } else {
                    currentLongest = 1;
                }
        ...
        document.getElementById('stats-total-days').textContent = `${uniqueHistory.length} day${uniqueHistory.length !== 1 ? 's' : ''}`;
    }
    ...
    const chartPercentageLabel = document.getElementById('chart-percentage');
    ...
    subjectContainer.appendChild(subjectCard)});const overallPercentage=totalChapters===0?0:Math.round((completedChapters/totalChapters)*100);overallProgressBar.style.width=`${overallPercentage}%`;overallProgressText.textContent=`${overallPercentage}%`;const angle=overallPercentage*3.6;performanceChart.style.background=`conic-gradient(var(--primary-color) ${angle}deg, var(--chart-track-color) ${angle}deg)`;chartPercentageLabel.textContent=`${overallPercentage}%`}

    // --- DATA PERSISTENCE ---
    function saveData() { localStorage.setItem('gateTrackerData', JSON.stringify(appData)); }
    function loadData() {
        const savedData = localStorage.getItem('gateTrackerData');
        ...
        if (!appData.customMinutes) appData.customMinutes = 25;
        if (!appData.studySessions) appData.studySessions = [];
        if (!appData.workouts) appData.workouts = [];
        if (!appData.loginHistory) appData.loginHistory = [];
        ...
        subjectNames.forEach(name => {
            if (appData.subjects[name]) { if (!Array.isArray(appData.subjects[name].chapters)) appData.subjects[name].chapters = []; }
            else { appData.subjects[name] = { chapters: [], revisionStage: 0, dueDate: null }; }
        });
        customMinutesInput.value = appData.customMinutes;
        setTimer();
        renderTodos();
        runAllRenders();
    }
