document.addEventListener('DOMContentLoaded', () => {
  ...
  measurementId: "G-LD1NMTVL0C"
    ...
    function startTimer() { pauseTimer(); const n = Date.now(), t = n + timeLeft * 1000; displayTimeLeft(timeLeft); countdown = setInterval(() => { const s = Math.round((t - Date.now()) / 1000); if (s < 0) { clearInterval(countdown); alertSound.play(); alert("Time for a break!"); logStudySession(appData.customMinutes * 60); setTimer(); return } timeLeft = s; displayTimeLeft(s) }, 1000) }
    ...
    // Activity Log
    function updateLoginStats(){const history=[...new Set((appData.loginHistory||[]))].map(d=>new Date(d)).sort((a,b)=>a-b);const uniqueDays=history.length;if(uniqueDays===0){["stats-current-streak","stats-longest-streak","stats-total-days"].forEach(id=>document.getElementById(id).textContent="0 days");return}
    let longestStreak=0,currentStreak=0,tempStreak=1;if(uniqueDays>0)longestStreak=1;for(let i=1;i<uniqueDays;i++){const diff=(history[i]-history[i-1])/(1000*3600*24);if(diff===1){tempStreak++}else if(diff>1){tempStreak=1}
    if(tempStreak>longestStreak)longestStreak=tempStreak}
