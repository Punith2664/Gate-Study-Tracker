document.addEventListener('DOMContentLoaded', () => {
    // The firebaseConfig object is GONE from here. It will be loaded from config.js
    ...
    // --- UI & RENDER FUNCTIONS ---
    function runAllRenders() { /* ... function bodies pasted below ... */ }
    ...
    runAllRenders = function() { renderSubjectsAndProgress(); renderRevisionsDue(); updateStatsDisplay(); updateWorkoutDisplay(); updateLoginStats(); updateWorkoutStats(); }
    displayTimeLeft = function(s) { const m = Math.floor(s / 60), r = s % 60; timerDisplay.textContent = `${m}:${r < 10 ? '0' : ''}${r}`; document.title = `${m}:${r < 10 ? '0' : ''}${r}` }
    setTimer = function() { pauseTimer(); appData.customMinutes = parseInt(customMinutesInput.value) || 25; timeLeft = appData.customMinutes * 60; displayTimeLeft(timeLeft); saveData(); }
    startTimer = function() { pauseTimer(); const n = Date.now(), t = n + timeLeft * 1000; displayTimeLeft(timeLeft); countdown = setInterval(() => { const s = Math.round((t - Date.now()) / 1000); if (s < 0) { clearInterval(countdown); alert("Time for a break!"); logStudySession(appData.customMinutes * 60); setTimer(); return } timeLeft = s; displayTimeLeft(s) }, 1000) }
    pauseTimer = function() { clearInterval(countdown) }
    renderTodos = function() { todoList.innerHTML='';(appData.todos||[]).forEach((task,index)=>{const li=document.createElement('li');li.className='list-group-item d-flex justify-content-between align-items-center';li.innerHTML=`<div class="form-check"><input class="form-check-input" type="checkbox" id="todo-${index}" ${task.completed?'checked':''}><label class="form-check-label ${task.completed?'text-decoration-line-through text-muted':''}" for="todo-${index}">${task.text}</label></div><button class="btn-close ms-2"></button>`;li.querySelector('.form-check-input').onchange=()=>{appData.todos[index].completed=!appData.todos[index].completed;renderTodos();saveData()};li.querySelector('.btn-close').onclick=()=>{appData.todos.splice(index,1);renderTodos();saveData()};todoList.appendChild(li)})}
    addTask = function() { const t=taskInput.value.trim();if(t==='')return;(appData.todos=appData.todos||[]).push({text:t,completed:false});taskInput.value='';renderTodos();saveData() }
    updateWorkoutDisplay = function() { const t = new Date().toISOString().slice(0, 10); workoutCheckbox.checked = (appData.workouts || []).includes(t) }
    ...
    const today=new Date(new Date().toISOString().slice(0,10));const lastWorkout=history[uniqueDays-1];if((today-lastWorkout)/(1000*3600*24)<2){currentStreak=tempStreak}
    document.getElementById('stats-workout-current').textContent=`${currentStreak} day${currentStreak!==1?'s':''}`;document.getElementById('stats-workout-longest').textContent=`${longestStreak} day${longestStreak!==1?'s':''}`;document.getElementById('stats-workout-total').textContent=`${uniqueDays} day${uniqueDays!==1?'s':''}`}
    updateLoginStats = function(){const history=[...new Set((appData.loginHistory||[]))].map(d=>new Date(d)).sort((a,b)=>a-b);const uniqueDays=history.length;if(uniqueDays===0){["stats-current-streak","stats-longest-streak","stats-total-days"].forEach(id=>document.getElementById(id).textContent="0 days");return}
    let longestStreak=0,currentStreak=0,tempStreak=1;if(uniqueDays>0)longestStreak=1;for(let i=1;i<uniqueDays;i++){const diff=(history[i]-history[i-1])/(1000*3600*24);if(diff===1){tempStreak++}else if(diff>1){tempStreak=1}
    if(tempStreak>longestStreak)longestStreak=tempStreak}
